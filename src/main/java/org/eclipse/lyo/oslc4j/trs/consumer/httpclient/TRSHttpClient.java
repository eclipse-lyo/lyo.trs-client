...
public class TRSHttpClient extends JazzFormAuthClient {
    ...
    private static Logger log = LoggerFactory.getLogger(TRSHttpClient.class);
    ...
    public Object fetchResourceUsingBaseAuth(String url, Class<?> objClass, String userName, String pwd)
        ...
        if (AbstractResource.class.isAssignableFrom(objClass)) {
            ...
            log.debug("getting entity of type: " + objClass.getName() + " for server reponse for resource: " + url);
            objToRet = clResp.getEntity(objClass);
            log.debug(
                    "finished getting entity of type: " + objClass.getName() + " for server reponse for resource: " +
                            url);
    ...
    public ClientResponse fetchResourceUsingBasicAuth(String url, String userName, String pwd) {
        log.debug("sending GET request to retrieve: " + url + " using basic credentials, user: " + userName);
...
//        requestHeaders.put("Accept", "application/rdf+xml, application/xml;q=0.6, text/xml;q=0.6");
    ...
    public static Model extractModelFromResponse(String absoluteUrl, final ClientResponse clientResponse)
        ...
        log.debug(
                "Found entity of type String in the sever response for url: " + absoluteUrl + ". Creating a Jena " +
                        "Model and returning it.");
        ...
        log.debug("successful created Jena model from response for url: ." + absoluteUrl + " . Returning the response");

        if (!rdFModel.isEmpty()) {
            log.debug(
                    "model extracted from response for url: ." + absoluteUrl + " has size: " + rdFModel.size() + " "
                            + "statement");
        }
