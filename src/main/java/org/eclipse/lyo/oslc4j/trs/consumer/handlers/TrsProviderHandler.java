    ...
    public List<ChangeLog> fetchUpdatedChangeLogs(TrackedResourceSet updatedTrs)
        ...
        if (!foundSyncEvent) {
            ...
            throw new ServerRollBackException(
                    "The sync event can not be found. The sever provinding the trs at: " + trsUriBase + " seems to "
                            + "have been rollecd back to a previous state");
        }
        return changeLogs;
    }
...
//        firstChangelogEvents = firstChangeLog.getChange();
    ...
    protected TrackedResourceSet extractTrsFromRdfModel(Model rdFModel) throws JenaModelException, URISyntaxException {
        ...
        try {
            trackedResourceSets = JenaModelHelper.fromJenaModel(rdFModel, TrackedResourceSet.class);
                ...
                InstantiationException | NoSuchMethodException | URISyntaxException | OslcCoreApplicationException e) {
            throw new JenaModelException(e);
        }

        TrackedResourceSet trs = null;

        if (isNotEmptySingletonArray(trackedResourceSets) && trackedResourceSets[0] instanceof TrackedResourceSet) {
     ...
     * @return trs pojo
     */
    protected TrackedResourceSet extractRemoteTrs()
            throws OAuthException, IOException, URISyntaxException, JenaModelException {
     ...
     * @return change log pojo
     */
    protected ChangeLog fetchRemoteChangeLog(String changeLogURl)
            throws IOException, OAuthException, URISyntaxException, IllegalArgumentException, SecurityException,
            JenaModelException {
     ...
     * @return base pojo
     */
    protected Base fetchRemoteBase(String baseUrl)
            throws OAuthException, IOException, URISyntaxException, JenaModelException {
